defmodule PhoenixAssetPipeline.HTML.Minifier do
  ...
  defp take_quoted_attr_value(rest, quote, _), do: take_quoted_attr_value(rest, quote, 0, byte_size(rest))
  ...
  defp take_unquoted_attr_value(rest, _), do: take_unquoted_attr_value(rest, 0, byte_size(rest))
  ...
  defp attribute_fragment?(static) do
    attribute_fragment?(static, not contains_angle_bracket?(static))
  end

  defp attribute_fragment?(<<first, rest::binary>>, true) when ascii_whitespace?(first) do
    rest = skip_ascii_whitespace(rest)
    {name, _} = take_attr_name(rest)
    valid_attr_name?(name)
  end

  defp attribute_fragment?(_, _), do: false
